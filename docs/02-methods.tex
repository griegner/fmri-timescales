...
\noindent Differentiating this quadratic function yields a linear equation in $\phitd$, and solving this results in a closed-form expression for the optimal $\phitd^*$. Therefore, $\phitd^*$ is defined by \textit{linear projection} and the timescale parameter $\tautd^*$ by a change of variable:
\begin{align}
    \phitd^* &= (\mathbb{E}[X_{t-1}^2])^{-1}(\mathbb{E}[X_t X_{t-1}]) \label{eq:ar1-phi},\\
    \tautd^* &= g(\phitd^*) = -\frac{1}{\text{log}(|\phitd^*|)}. \label{eq:ar1-tau}
\end{align}

\noindent Thus, the timescale represents the characteristic time of $1/e$ autocorrelation decay derived from the best-fitting AR1 model under squared error loss. Since $X_t$ is stationary with finite variance, the parameters $\phitd^*$ and $\tautd^*$ defined by projection are unique; in fact, any approximating AR1 model is identifiable if $\mathbb{E}[X_{t-1}^2]$ is nonzero and finite \citep[Theorem~14.28]{hansen_econometrics_2022}.\\

The parameter $\phitd^*$ then represents the best linear, mean-square approximation of the process $X_t$ by an AR1 model. Importantly, the observed process $X_t$ need not follow the AR1 model from equation~\eqref{eq:ar1} with \textbf{iid} innovations $e_t$. This allows for projection errors $e_t^* = X_t - \phitd X_{t-1}$, i.e. residuals from linear projection onto the working model, in which deviations from AR1 are subsumed by $e_t^*$ which may exhibit unequal variance and autocorrelation. This applies to any stationary and mixing process, even if the true data-generating process is not AR1, so the resulting fit an AR1 projection.
