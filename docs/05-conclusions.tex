...
In the fMRI setting, an autoregressive process is unlikely to generate the observed time series. Instead, is it more plausible that it is really the sum of several distinct stochastic processes, at the very least one capturing neural dynamics, and another capturing measurement noise. In such settings, regions with identical neural timescales can nonetheless yield different apparent fMRI timescales under varying amounts of measurement noise. This raises an interpretational concern -- the fitted timescale may be well-estimated yet not correspond to the intended index of neural integration if measurement noise dominates. \\

To motivate this limitation, consider the simulation setting where the latent neural process is generated as an AR1 process with a fixed timescale $\tautd^* = 4.48$ ($\phitd^* = 0.8$):
...
\noindent to vary observation quality while holding the latent timescale constant. Under this signal + noise construction, increasing $\sigma^2_\epsilon$ increases the rate of autocorrelation decay (\textbf{Figure \ref{fig:sim-noise} Panel A}), because it inflates only the lag-0 variance without changing cross-lag covariances, so the ratio that defines the autocorrelation in \eqref{eq:acf} shrinks. As a consequence of increasing measurement noise, the time-domain estimator in \textbf{Panel B row 1} increasingly underestimates the theoretical latent neural timescale, even though the robust standard errors are correct in \textbf{Panel B row 2}. This perspective emphasizes that denoising the fMRI time series before estimating neural timescales is nontrivial to the interpretation of timescale maps; especially when making inference about neural timescales separate from other sources of variability like measurement or physiological noise.\\

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{docs/figures/fig07-rshrf.pdf}
    \caption{\textit{CAPTION.}}
    \subcaption*{SUBCAPTION}
    \label{fig:map-rshrf}
\end{figure}

Take for example the hemodynamic response function ...
...
Monotonic autocorrelation decay...
