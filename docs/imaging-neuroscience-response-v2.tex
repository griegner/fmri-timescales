...
\begin{itemize}
    ...
    \item \textbf{Appendix for the Autocorrelation-Domain Score Process:} To further clarify the construction of $u_t$ and $\omegaad$, we have added an appendix that derives the autocorrelation-domain score process directly from the gradient of the objective function (Eq. 5) and shows how this leads to the sandwich form in Eq. 16.
\end{itemize}
...
We appreciate the reviewer’s concern about accessibility and agree that the earlier version contained a rather dense methods section. The methods section is now more concise because we focus on only two estimation approaches---the time-domain and autocorrelation-domain methods---without the additional hybrid variants that previously required separate sections. We have also added an appendix deriving the autocorrelation-domain score process, so that some of the more technical steps are available without overloading the main text.

To further improve readability, we removed equation numbers from expressions that are never referenced, so that readers can safely skip unnumbered equations without loss of continuity. Following the reviewer’s suggestion, we have added the following paragraph at the end of the introduction that directs readers to the most relevant sections according to their interests. We would be happy to further restructure the sections if the other reviewers or the AE/editor suggest it.

\manuscriptquote{The proposed methods address important limitations in fMRI timescale research by providing rig-
orous statistical methods that move beyond point estimates to incorporate uncertainty quantification.
This work establishes a methodological foundation for reliable inference, organized as follows. First,
Section 2 formalizes the statistical models and their properties. Numbered equations indicate impor-
tant expressions that are referenced throughout, while the others provide intermediate steps that can
be skipped without loss of continuity. These models are validated against varying degrees of realistic
misspecification in Section 3, followed by a demonstration of their empirical utility in Section 4 using
the Human Connectome Project dataset. Finally, Section 5 addresses practical considerations for the
interpretation of these estimates in the context of fMRI --- detailing the influence of hemodynamics,
measurement noise, and periodicity --- to support future research investigating the functional and
structural organization of timescales in the brain.}

\textbf{4 Minor Issues}
\reviewercomment{Below are very minor issues, including typos, where I indicate the location in the draft and the issue I noticed.}
